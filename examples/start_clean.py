...
async def main() -> int:
    ...
    try:
        ...
        await client.start_clean(device_name)
    except NeakasaError as exc:
        print(f"SDK error: {exc}", file=sys.stderr)
        return 1

    print(f"Clean cycle started on {device_name}.")
    return 0
