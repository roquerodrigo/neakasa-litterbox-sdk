"""Toggle the device's child lock.
...
``off`` (default ``on``) to disable. Run with::
    ...
    uv run python examples/set_child_lock.py
    uv run python examples/set_child_lock.py off
"""
...
async def main() -> int:
    ...
    try:
        ...
        await client.set_child_lock(device_name, enabled)
    except NeakasaError as exc:
        print(f"SDK error: {exc}", file=sys.stderr)
        return 1

    print(f"Child lock {'enabled' if enabled else 'disabled'} on {device_name}.")
    return 0
