...
async def main() -> int:
    ...
    try:
        ...
        await client.stop_level(device_name)
    except NeakasaError as exc:
        print(f"SDK error: {exc}", file=sys.stderr)
        return 1

    print(f"Leveling pass stopped on {device_name}.")
    return 0
